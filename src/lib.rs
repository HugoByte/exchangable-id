...
pub trait ExchangableIdProvider<T: frame_system::Trait> {
	fn get_uuid(exchangable_id: ExchangableId<T>) -> Option<Vec<u8>>;
	fn check_uuid_exists(uuid: &UUID) -> bool;
	fn check_exchangable_id_owner(owner: T::AccountId, exchangable_id: ExchangableId<T>) -> bool;
}

impl<T: Trait> ExchangableIdProvider<T> for Module<T> {
	fn get_uuid(exchangable_id: ExchangableId<T>) -> Option<Vec<u8>> {
		let uuid = ExchangableIds::<T>::get(exchangable_id);
		return uuid;
	}

	fn check_uuid_exists(uuid: &UUID) -> bool {
		let exchangable_id = T::Hashing::hash_of(&Uuid::parse_str(str::from_utf8(&uuid).unwrap()).unwrap().as_bytes());
		ExchangableIds::<T>::contains_key(&exchangable_id)
	}

	fn check_exchangable_id_owner(owner: T::AccountId, exchangable_id: ExchangableId<T>) -> bool {
		debug::info!("owner {:?}  check_exchangable_id_owner {:?} {:?}", owner, (&owner, Some(exchangable_id)), Some(exchangable_id));
		<OwnedExchangableIds<T>>::contains_key((&owner, Some(exchangable_id)))
	}
}
